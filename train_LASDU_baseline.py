...
from utils.trainer_IKD_Common import ModelTrainer
...
class DalesConfig(Config):
    ...
    # Dataset name
    dataset = 'lasdu_500_1_5'

    AddMessage='baseline' #额外的信息
    ...
    first_subsampling_dl =0.6 #0.6     #0.25
...
if __name__ == '__main__':
    ...
    # Initialize datasets
    training_dataset = CommonDataset(config, set='training', use_potentials=True)
    test_dataset = CommonDataset(config, set='validation', use_potentials=True)

    # Initialize samplers
    training_sampler = CommonSampler(training_dataset)
    ...
    trainer = ModelTrainer(net, config, cudaDevice, chkp_path=chosen_chkp)
    print('Done in {:.1f}s\n'.format(time.time() - t1))
