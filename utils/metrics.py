...
def iou(confusions,num_classes):
    ...
    for label in range(num_classes):
        tp = np.longlong(confusions[label, label])
        fn = np.longlong(confusions[label, :].sum()) - tp
        fp = np.longlong(confusions[:, label].sum()) - tp
...
def OA(confusions):
    oacc = np.diag(confusions).sum() / confusions.sum()
    return oacc

def F1_score(confusions):
    precision_cls = np.diag(confusions) / np.sum(confusions, axis=0)
    recall_cls = np.diag(confusions) / np.sum(confusions, axis=1)
    f1_scores = ((2 * precision_cls * recall_cls) / (precision_cls + recall_cls)).tolist()
    f1_scores.append(np.nanmean(f1_scores))
    return f1_scores

def acc(confusions,num_classes):
